...
def work_node(work, facet=None):
    ...
    for ebook in ebooks:
        ...
        if not ebook.version_label in versions:
            ...
            link_node.attrs.update({"href":add_query_component(ebook.download_url, "feed=opds"),
                                         "rel":link_rel,
    ...
    cover_node.attrs.update({"href":work.cover_image_small(),
                              "type":"image/"+work.cover_filetype(),
                              "rel":"http://opds-spec.org/image/thumbnail"})
    node.append(cover_node)
    cover_node = soup.new_tag("link")
    cover_node.attrs.update({"href":work.cover_image_thumbnail(),
                              "type":"image/"+work.cover_filetype(),
        ...
        if subject.is_visible:
            ...
            try:
                ...
                try:
                    ...
                    node.append(navlink('related', 'kw.'+ subject.name , 0, 'popular', title=subject.name))
                except:
                    pass
            except ValueError:
...
def append_navlink(feed, rel, path, page, order_by, group=None, active=None , title=""):
    feed.append(navlink(rel, path, page, order_by, group, active, title))

def navlink(rel, path, page, order_by, group=None, active=None , title=""):
    link = soup.new_tag("link")
    link.attrs.update({"rel":rel,
