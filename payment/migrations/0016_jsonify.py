...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for t in orm['payment.Transaction'].objects.all():
            t.extra={}
            if t.ack_name:
                t.extra.update({"ack_name": t.ack_name})
            if t.ack_dedication:
                t.extra.update({"ack_dedication": t.ack_dedication})
            t.save()

    def backwards(self, orm):
        premiums=orm['core.Premium']
        transactions=orm['payment.Transaction'].objects.all()
        for t in transactions:
            if t.extra is None:
                t.extra={}
            t.ack_name=t.extra.get("ack_name", None)
            t.ack_dedication=t.extra.get("ack_dedication", None)
            t.save()
