...
Vendored from gitberg 0.8.7 (https://github.com/gitenberg-dev/gitberg, GPLv3,
Copyright 2012-2024 Seth Woodworth and the Free Ebook Foundation):
`gitenberg/metadata/pandata.py`, with `plural()` inlined from
`gitenberg/metadata/utils.py`. Vendored so that importing Pandata no longer
executes `gitenberg/__init__.py`, whose import chain performs a network fetch
at import time and can poison mod_wsgi workers at startup.
See https://github.com/Gluejar/regluit/issues/1204
'''
import re
import six
import yaml
...
def plural(key):
    # inlined from gitenberg.metadata.utils
    if key.endswith('s'):
        return key+'es'
    else:
        return key+'s'
