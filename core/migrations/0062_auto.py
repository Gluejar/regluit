...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding index on 'Campaign', fields ['status']
        db.create_index('core_campaign', ['status'])

        # Adding index on 'Campaign', fields ['activated']
        db.create_index('core_campaign', ['activated'])

        # Adding index on 'Campaign', fields ['left']
        db.create_index('core_campaign', ['left'])

        # Adding index on 'Work', fields ['language']
        db.create_index('core_work', ['language'])

        # Adding index on 'Work', fields ['created']
        db.create_index('core_work', ['created'])

        # Adding index on 'Work', fields ['featured']
        db.create_index('core_work', ['featured'])

        # Adding index on 'Press', fields ['date']
        db.create_index('core_press', ['date'])

        # Adding index on 'Ebook', fields ['created']
        db.create_index('core_ebook', ['created'])

        # Adding index on 'Wishes', fields ['source']
        db.create_index('core_wishlist_works', ['source'])

        # Adding index on 'Wishes', fields ['created']
        db.create_index('core_wishlist_works', ['created'])

        # Adding index on 'Acq', fields ['created']
        db.create_index('core_acq', ['created'])

        # Adding index on 'Edition', fields ['publication_date']
        db.create_index('core_edition', ['publication_date'])


    def backwards(self, orm):
        # Removing index on 'Edition', fields ['publication_date']
        db.delete_index('core_edition', ['publication_date'])

        # Removing index on 'Acq', fields ['created']
        db.delete_index('core_acq', ['created'])

        # Removing index on 'Wishes', fields ['created']
        db.delete_index('core_wishlist_works', ['created'])

        # Removing index on 'Wishes', fields ['source']
        db.delete_index('core_wishlist_works', ['source'])

        # Removing index on 'Ebook', fields ['created']
        db.delete_index('core_ebook', ['created'])

        # Removing index on 'Press', fields ['date']
        db.delete_index('core_press', ['date'])

        # Removing index on 'Work', fields ['featured']
        db.delete_index('core_work', ['featured'])

        # Removing index on 'Work', fields ['created']
        db.delete_index('core_work', ['created'])

        # Removing index on 'Work', fields ['language']
        db.delete_index('core_work', ['language'])

        # Removing index on 'Campaign', fields ['left']
        db.delete_index('core_campaign', ['left'])

        # Removing index on 'Campaign', fields ['activated']
        db.delete_index('core_campaign', ['activated'])

        # Removing index on 'Campaign', fields ['status']
        db.delete_index('core_campaign', ['status'])
    ...
    models = {
        ...
        'core.acq': {
            ...
            'refreshes': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 7, 31, 0, 0)', 'auto_now_add': 'True', 'blank': 'True'}),
